...
int main(int argc, char** argv) {
	...
	bank1->balance = 2500;
	bank1->mutex = (pthread_mutex_t*) malloc(sizeof(pthread_mutex_t));
	...
	bank2->balance = 3000;
	bank2->mutex = (pthread_mutex_t*) malloc(sizeof(pthread_mutex_t));
	...
	for (int i=0; i < 1000; ++i) pthread_join(pool[i], NULL);
